...
- `POST /api/query`: 30 requests per minute per authenticated user or API key
- Table metadata endpoints: 60 requests per minute
- `POST /api/assistant/generate/stream`: 20 requests per minute (streaming SSE)
